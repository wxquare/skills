...
**If the user asks for an ER diagram, data model diagram, database schema diagram, or table relationship diagram**: read `references/erd-diagram.md` before designing. ERDs need a specialized preparation pass: table grouping, field pruning, relationship strength classification, and a highlighted happy path. Do not draw every foreign-key-like field as a full mesh.
...
Settings: `fontSize: 16`, `fontFamily: 8` (`Comic Shanns`, use this by default), `textAlign: "center"`, `verticalAlign: "middle"`
...
17. **Font**: `fontFamily: 8` (`Comic Shanns` by default)
